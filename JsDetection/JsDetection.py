...
def isJs(jsDir, parser = 'esprimaAst'):
	...
	#subprocess.run(['slimit', '--mangle', inputFile], stdout = subprocess.PIPE).stdout.decode('utf-8');
	...
	if parser.lower() == 'slimit':
		...
		for givenFile in sorted(l):
			#result = subprocess.run(['node' , '../src/JsEsprima/parser.js', givenFile], stdout = subprocess.PIPE).stdout.decode('utf-8');
			#if result == '':
				#print('The file ' + givenFile + ' is not considered as a valid JavaScript file.');
