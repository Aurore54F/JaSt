...
def classify(trainingFile, modelDir=currentPath+'/Classification/',\
    ...
    labels = data['Label'] # Vector containing all the labels (i.e. 'benign', 'malicious',
        #or '?') of the samples considered
    ...
    clf.predict_proba(X) # Returns the probability of the samples for each class in the model.
        #The columns correspond to the classes in sorted order, as they appear in the
        #attribute classes

    labelsPredicted = clf.predict(X) # Perform classification on an array of test vectors X and
        #predict the target values
    accuracy = clf.score(X, labels) # Detection rate

    if printScore:
        TN, FP, FN, TP = confusion_matrix(labels, labelsPredicted).ravel()
        # y = labels, predicted = labels predicted
...
def validate(validationFile, model, modelDir=currentPath+'/Classification/',\
             modelName='/model'):
    '''
        Extension of a Naive Bayes Multinomial classification model with a new CSV file of
        the same format.
        ...
        (i.e. same attributes) as the format of validationFile.
    ...
    validated = model.partial_fit(XValidation, labelsValidation, ['malicious', 'benign'])
...
def testModel(testFile, model, printRes=True):
    '''
        Test of a Naive Bayes Multinomial classification model with a new CSV file of the
        same format.
        ...
        - model
            Naive Bayes Multinomial model.
        Beware: the model must have been constructed using files of the same format
        (i.e. same attributes) as the format of testFile.

        -------
        Returns:
        - Naive Bayes Multinomial model.
            Beware: the model was implemented as a global variable in sklear.
        - Print the detection rate and the TP, FP, FN and TN rates of testFile
        tested with model, in stdout.
    '''
    ...
    labelsPredictedTest = model.predict(XTest) # Perform classification on an array of test
        #vectors X and predict the target values
    labelsPredictedProbaTest = model.predict_proba(XTest) # Perform classification on an array of
        #test vectors X and predict the target values
    accuracyTest = model.score(XTest, labelsTest) # Detection rate

    if printRes:
        for i in range(0, len(names)):
            print(str(names[i]) + ': ' + str(labelsPredictedTest[i]) + ' ('\
                  + str(labelsTest[i]) + ')')
        print('> Name: labelPredicted (trueLabel)')
...
def crossValidation(trainingFile, n_splits=10):
    ...
    labels = data['Label'] # Vector containing all the labels (i.e. 'benign', 'malicious', or '?')
        #of the samples considered
    X = data.ix[:, '0':] # 2D vector containing all the attributes of the samples considered
    X = np.asarray(X)
    ...
    for train, test in cv.split(X, labels):
        ...
        TN_test, FP_test, FN_test, TP_test = confusion_matrix(labels[test], labelsPredicted).ravel()
        # y = labels, predicted = labels predicted

        print("Detection: " + str(accuracyTest))
        print("TN: " + str(TN_test) + ", FP: " + str(FP_test) + ", TP: " + str(TP_test) + ", FN: "\
              + str(FN_test))

        fpr, tpr, thresholds = roc_curve(labels[test], labelsPredictedProba[:, 0],\
                                         pos_label='benign')
