...
def simplifyEsprimaSyntacticalTokens():
    ...
    expressions = [m[i].split("'")[1] for i in range(len(m))]
    # List containing the Esprima expression units present in the string "Syntax" above.

    m = re.findall("\'[A-z]*Element\'", Syntax)
    elements = [m[i].split("'")[1] for i in range(len(m))]
    # List containing the Esprima element units present in the string "Syntax" above.

    m = re.findall("\'[A-z]*Statement\'", Syntax)
    statements = [m[i].split("'")[1] for i in range(len(m))]
    # List containing the Esprima statement units present in the string "Syntax" above.

    m = re.findall("\'[A-z]*Literal\'", Syntax)
    literals = [m[i].split("'")[1] for i in range(len(m))]
    # List containing the Esprima literal units present in the string "Syntax" above.

    m = re.findall("\'[A-z]*Declaration\'", Syntax)
    declarations = [m[i].split("'")[1] for i in range(len(m))]
    # List containing the Esprima declaration units present in the string "Syntax" above.

    m = re.findall("\'[A-z]*Pattern\'", Syntax)
    patterns = [m[i].split("'")[1] for i in range(len(m))]
    # List containing the Esprima pattern units present in the string "Syntax" above.

    m = re.findall("\'[A-z]*Specifier\'", Syntax)
    specifiers = [m[i].split("'")[1] for i in range(len(m))]
    # List containing the Esprima specifier units present in the string "Syntax" above.

    comments = ['LineComment', 'BlockComment']
    # List containing the Esprima syntactical comment tokens.
    ...
    simplifiedList = [expressions, elements, statements, literals, declarations, patterns,\
                      specifiers, comments, program, property2, super2, switchCase,\
                      variableDeclarator, catchClause, classBody, identifier, import2,\
...
def buildAstSimplifiedDicoEsprima():
    '''
        ...
        As the list of units has been simplified, different units can be mapped to the same int.
    ...
    for l in simplifiedList:
        for el in sorted(l):
            dico[el] = i # Every elements el in a given list l will be mapped to the same integer,
...
tokensDico = { \n"
    ConfFileProduction.dicoStorage(currentPath+'/Dico_MapTokens-Int',\
                                   'DicoOfAstEsprimaSimplified.py', descr, orderedDico)
